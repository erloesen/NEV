...
const initMap = () => {
  map = L.map('map');
  map.setView([39.900497, 116.370491], 10);
...
const showSelectedFac = async (id) => {
  ...
  const url = "http://172.16.206.91:8080/geoserver/nev/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=nev%3Acompany_polygon_relation&maxFeatures=500&outputFormat=application%2Fjson"
  ...
  map.fitBounds(dataMap.工厂围栏.getBounds(), { padding: [100, 100] });
}

const showImage = (id, imgdate) => {
  dataMap.影像.clearLayers()
  const satellite = L.tileLayer.wms("http://172.16.206.91:8080/geoserver/nev/wms", {
    format: "image/png",
    version: "1.1.0",
    layers: `nev:${id}`,
